...
initGame : GameState
initGame = MkGameState 0 1 0 0 (0, cast (0.4 * windowHeight)) True
...
showGame = do
    (MkGameState score speed lastTime streak (x,y) dir past size) <- get
...
e2w (E_KEYDOWN EK_SPACE) = do
    ...
    case past of
        (((px, py), psize) :: ls) => (do
            ...
            put $ (MkGameState (score + 1) speed lastTime streak (cx, cy) (not dir) (shiftDown (((xpos, y), csize) :: past)) csize))
        _ => ?idk
e2w (E_KEYDOWN EK_RETURN) = put initGame
e2w e = pure ()

updatePos : Double -> Int -> Bool -> Int -> Int -> (Int, Bool)
updatePos dt x True speed size = (x + cast ((cast speed) * dt), x + cast ((cast speed) * dt) + size < windowWidthInt)
updatePos dt x False speed size = (x - cast ((cast speed) * dt), x - cast ((cast speed) * dt) < 0)
...
t2w t = do
    (MkGameState score speed lastTime streak (x,y) dir past size) <- get
    if lastTime == 0
      then put $ (MkGameState score speed t streak (x, y) dir past size)
      else do
        let deltaTime = t - lastTime
        let (x, dir) = updatePos (deltaTime * 500) x dir speed size
        put $ (MkGameState score speed t streak (x, y) dir past size)
